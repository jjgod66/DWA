-- sal 를 1000으로 나눘을때의 나머지 ==> MOD 함수, 별도의 연산자는 없다.
...
SELECT SYSDATE now, SYSDATE -3 now_before,
	   TO_DATE('2023/12/31', 'YYYY/MM/DD') lastday,
	   TO_DATE('2023/12/31', 'YYYY/MM/DD') -5 lastday_before
  FROM dual;
...
SELECT TO_CHAR(SYSDATE, 'YYYY-MM-DD') dt_dash,
	   TO_CHAR(SYSDATE, 'YYYY-MM-MM HH24-MI-SS') dt_dash_with_time,
...
-- LAST_DAT(date1) : date1 날짜가 속한 월의 마지막 날짜를 반환
...
SELECT ename, sal, TO_CHAR(sal, '9,999') fm_sal
  FROM emp;

SELECT TO_CHAR(10, 'FM9990.90')
  FROM dual;
-- FM9990,99 의 의미
-- 9는 해당자리의 숫자를 의미하고 값이 없을 경우 소수점 이상은 공백으로, 소수점 이하는 0으로 표시한다.
...
-- 실습4
-- 위 실습내용에서 mgr 컬럼의 NULL 처리를 nvl2, coalesce 를 사용하여 조회하는 쿼리를 작성해주세요.
SELECT empno, ename, mgr, NVL(mgr, 111) mgr_n, NVL2(mgr, mgr, 111) mgr_n_1,
       COALESCE(mgr, 111) mgr_n_2
  FROM emp;
