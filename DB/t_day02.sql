...
-- emp테이블에서 sal 값이 1000보다 크거나 같고 2000보다 작거나 같은 사원들만(행들만) 조회해주세요.
SELECT *
  FROM emp
 WHERE sal >= 1000
   AND sal <= 2000
...
-- emp 테이블에서 deptno(부서번호) 10 혹은 20 인 사원들을 조회해주세요. OR 혹은 IN 연산자 사용
SELECT *
  FROM emp
...
-- member 테이블에서 회원의 성이 [신]씨인 사람의 mem_id, mem_name 을 조회하는 쿼리를 작성해주세요.
SELECT *
  FROM MEMBER
