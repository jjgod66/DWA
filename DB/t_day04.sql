...
SELECT ROWNUM, a.*
  FROM (SELECT empno, ename
  		  FROM emp
  		 ORDER BY ename) a;
-- ==> 실행순서 : 바깥쪽 FROM => 안쪽 FROM => 안쪽 SELECT => 안쪽 ORDER BY => 바깥쪽 SELECT
...
-- emp 테이블에서 사원 이름이 smith 인 사람을 조회하세요.
SELECT *
  FROM emp
;
