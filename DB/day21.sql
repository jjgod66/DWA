...
-- CREATE [OR REPLACE] FUNCTION 함수명([파라미터]) RETURN 리턴해줄타입 IS
-- BEGIN
--
-- END;

-- RETURN TYPE을 명시할 때 SIZE 정보는 명시하지 않음
-- RETURN VARCHAR2(2000) ==> X
-- RETURN VARCHAR2 		 ==> O
...
-- getEmpName, getDeptName을 names라는 이름의 패키지를 생성하여 등록
...
-- 감지시 실행 로직 : 변경 전 pass 값을 users_history 에 저장

CREATE OR REPLACE TRIGGER make_history
--	BEFORE UPDATE ON users
--	BEFORE UPDATE OF 컬럼명 ON users	-- ==> 해당컬럼이 업데이트되었을 때
	BEFORE INSERT OR UPDATE OR DELETE ON users
	FOR EACH ROW 
--	WHEN (:OLD.userid = 'brown')	-- ==> 바뀐 행의 userid가 'brown'일때
...
UPDATE users SET usernm = 'brown' WHERE userid = 'brown';
...
UPDATE users SET pass = '1234' WHERE userid = 'brown';	
...
--	PL/SQL 블록에서 예외가 발생하면 예외가 발생된 지점에서 코드 중단(에러)
-- 	EXCEPTION 절에 기술한 코드가 실행

-- 예외를 포장하기 => 예외를 잡아 사용자가 정의한 새로운 예외로 던지는 작업
SELECT ename
  FROM emp
 WHERE empno = -99999;
