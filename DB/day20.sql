...
-- java에서 변수 하나에 여러개의 값을 넣을 있는 자료형 : 배열, List, Map, Set ...
...
--					 행의 정보를 담을 수 있는 것은 ROWTYPE과 동일, 사용자가 원하는 컬럼에 대해서만 정의
...
BEGIN
	SELECT * INTO v_dept_row /*deptno, dname INTO v_dept_row.deptno, v_dept_row.dname*/
	  FROM dept
	 WHERE deptno = 10;
...
-- dept테이블의 10번 부서의 부서번호, 부서이름을 RECORD TYPE으로 생성된 변수에 값을 담아서 출력
...
-- emp 테이블에서 'SMITH'사원의 사원번호, 사원이름, 사원부서, 업무를 RECORD TYPE을 통해 출력하는 익명 PL/SQL 블럭을 작성하세요
DECLARE
...
BEGIN 
	SELECT empno, ename, dname, job INTO v_emp_rec
	  FROM emp, dept
...
-- CURSOR 의 종류
-- 묵시적 커서 : 커서 이름을 별도로 지정하지 않을 경우 ==> ORACLE이 알아서 처리해줌
