...
//		p2.setId(1);
		...
		System.out.println(p1 == p2);		// 참조값(번지)이 들어가는 것이기 때문에 false
...
//		System.out.println(persons.size());
		...
		persons.add(new Person("성춘향"));
		System.out.println(persons.size());
		// hashCode를 equals메서드와 함께 재정의하지 않으면 코드가 예상과 다르게 작동하는
		// 문제를 일으킨다. 정확히 말하면 hash값을 사용하는 Collection(HashSet, HashMap, HashTable)
		// 을 사용할 때 문제가 발생한다.
		// 왜 그럴까..? ==> hash값을 사용하는 Collection은 객체가 논리적으로 같은지 비교할 때 
...
class Person {
	...
	public int hashCode() {
		...
		// hash값을 어떻게 생성해야 균등한지 분포도가 좋은지는 알기 어려우며 거의 수학적 영역에 가깝다.
